...
class look_ahead():
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            # # xy coordinate origin adjust
            if seq == 0:
                x_origin_adj = self.waypoint_x[seq] - x
                y_origin_adj = self.waypoint_y[seq] - y
            else:
                x_origin_adj = self.waypoint_x[seq] - self.waypoint_x[seq-1]
                y_origin_adj = self.waypoint_y[seq] - self.waypoint_y[seq-1]

            theta = np.arctan2(y_origin_adj, x_origin_adj)
